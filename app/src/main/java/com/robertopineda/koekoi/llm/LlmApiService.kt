        ...
        **Instructions:**
        ...
        3.  **Output Format:** Provide ONLY a single JSON object matching the following structure. Do NOT include any explanation, commentary, or text outside the JSON block. Ensure the JSON is valid.
            ```json
            {
              ...
              "reading": "The phonetic reading of the phrase. For Japanese, provide the full Hiragana/Katakana reading (Furigana). For other languages (Korean, Vietnamese, Spanish), provide an empty string unless a standard, commonly used phonetic transcription system exists and is useful for learners (usually empty string is best).",
        ...
        5.  **`reading` Field Detail:** For Japanese, always provide the full reading in Hiragana/Katakana. For Spanish, Korean, Vietnamese, leave `reading` as `""` (empty string).
